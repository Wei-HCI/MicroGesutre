...
def data_aug(data, cls, state):
   ...
   # print(f"Augmented data shape: {skeleton_aug}")
   #  print(f"Augmented class labels shape: {cls_aug.shape}")
   #  print(f"Augmented state labels shape: {state_aug.shape}")
...
class SkeletonData(Dataset):
    def __init__(self, data, cls_labels, state_labels, mode="train", use_data_aug=False):
        ...
        # print(f"Class labels shape: {self.cls_labels.shape}")
        # print(f"State labels shape: {self.state_labels.shape}")
        assert len(data) == len(cls_labels) == len(
            state_labels), "Data and labels must have the same length after augmentation."
