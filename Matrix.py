...
def init_data_loader(args, logger, t_data, v_data=None):
    if v_data is None:
        ...
        print(f"Minimum label value: {train_label.min()}")
        print(f"Maximum label value: {train_label.max()}")
    ...
    print(f"Before SkeletonData - Data shape: {train_data.shape}")
    print(f"Before SkeletonData - Class labels shape: {train_label.shape}")
    print(f"Before SkeletonData - State labels shape: {train_state.shape}")
...
def initialise_model(args, model):
    class_num = 8  # including null class
    state_num = 5
    joint_num = 11
    input_dim = 7
    save_att = False
...
def get_cm(scores, label):
    ...
    if np.any(np.isnan(label)) or np.any(np.isnan(outputs)):
        print("NaN values found in label or outputs.")
        print(f"Label: {label}")
        print(f"Outputs: {outputs}")

    valid_indices = ~np.isnan(label) & ~np.isnan(outputs)
    label = label[valid_indices]
    outputs = outputs[valid_indices]
    m = confusion_matrix(label.flatten(), outputs.flatten(), normalize='true')  # Normalize the confusion matrix
    acc = accuracy_score(label.flatten(), outputs.flatten())
    return m, acc

def plot_confusion_matrix(cm, class_names, figure_path=None):
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt=".3f", cmap="viridis", xticklabels=class_names, yticklabels=class_names)
...
def train_model(arguments, model_type, name, t_data, v_data=None):
    if arguments.training:
        ...
        for epoch in range(arguments.epochs):
            ...
            with torch.no_grad():
                ...
                plot_confusion_matrix(val_cm, class_names=['Fist', 'Cube', 'Vertical', 'Ring', 'Pinky', 'Scissors','Slide','Null'])
                confusion_matrix_path = f'{epoch}_confusion_matrix.png'
    ...
    if arguments.inference:
        ...
        try:
            ...
            model_fold = f"{os.getcwd()}/120824_RH_model_20f_noise_1-2_256"
            best_model = "ep_4_acc_0.965_182831"
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(val_loader):
                ...
                print("Classification output for current batch:", cls)
                val_loss += loss
                if i == 0:
                    score_list = cls
                    label_list = label
            ...
            #print(f"{step_end - step_start:.4f}")
    ...
    if arguments.calibration:
        try:
            ...
            model_fold = f"{os.getcwd()}/120824_RH_model_20f_noise_1-2_256"
            best_model = "ep_3_acc_0.985_174814"
            model_path = f'{model_fold}/{best_model}.pt'
            logger = get_logger(model_fold + "/train.log")
            train_loader, val_loader = init_data_loader(arguments, logger, t_data, v_data)
        model = initialise_model(arguments, model_type)
...
if __name__ == "__main__":
    ...
    rh_tr_data = 'C:/Users/adminroot/AppData/LocalLow/HW/MicroGesture/11Joints/P*[!4]/*/*'
    #rh_v_data = 'C:/Users/adminroot/My project/Gesture_data/P*[3]/*/*RightHand*.csv'
    rh_v_data = 'C:/Users/adminroot/AppData/LocalLow/HW/MicroGesture/11Joints/P*[4]/*/*'
